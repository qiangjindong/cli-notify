...
with Client() as c:
    ...
    assert sum('bridge.py native' in h.get('command','') and h['trustStatus']=='trusted' for h in hooks)==5
    ...
    ident=__import__('hashlib').sha256(tid.encode()).hexdigest()[:32]
    ...
    send('forget')
    print('PASS: real question hook, answer continuation and completion hook. Window id:',ident)
